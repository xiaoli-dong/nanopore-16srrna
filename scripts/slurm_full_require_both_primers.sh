...
results_dir="${workdir}/results_full"
...
for var in {25..48} {73..96}; do
    ...
    #get rid of primers
    # 16s-27F - AGA GTT TGA TCM TGG CTC AG
    # 16S-DG74 - AAG GAG GTG ATC CAA CCG CA
    ...
    cmd="singularity run  ${progs}/cutadapt:4.4--py39hf95cd2a_1 cutadapt -j $SLURM_CPUS_PER_TASK -e 0.2 --overlap 10 --revcomp  -m ${minlen} -M ${maxlen} --max-n 1  -g "AGAGTTTGATCMTGGCTCAG...TGCGGTTGGATCACCTCCTTT" --untrimmed-output=${results_dir}/${prefix}/qc/cutadapt/${prefix}.cutadapt_untrimmed.fastq.gz -o ${results_dir}/${prefix}/qc/cutadapt/${prefix}.cutadapt_trimmed.fastq.gz ${results_dir}/${prefix}/qc/porechop/${prefix}.porechop.fastq.gz"
    echo $cmd
    echo ""
    #$cmd
